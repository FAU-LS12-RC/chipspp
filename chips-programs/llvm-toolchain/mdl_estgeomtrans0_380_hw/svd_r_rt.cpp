/*
 * SVD_R_RT - DSP System Toolbox Singular Value Decomposition helper functions
 *
 ...
 */
//LIBMWVISIONRT_API int_T MWVIP_SVD_R
int_T MWVIP_SVD_R
(
                   real32_T	*x,
                   int_T	 n,
    ...
    */
    ...
    for (l=0; l<lu; l++) {
        ...
        * and place the l-th diagonal in s(l).
            ...
            *psl = Zero;
            ...
            if (fabsf(*psl) != Zero) {
                if (fabsf(*pxll) != Zero) {
                    /*  *psl = sign(*psl,*pxll)  */
                    *psl = (*pxll >= Zero) ? fabsf(*psl) : -fabsf(*psl);
                    /*  *psl = sign(*psl,*pxll)  */
                }
        ...
        *	the l-th super-diagonal in e(l).
            ...
            *pel = Zero;
            ...
            if (fabsf(*pel) != Zero) {
                if (fabsf(*pel1) != Zero) {
                    /*  *pel = sign(*pel,*pel1)  */
                    *pel = (*pel1 >= Zero) ? fabsf(*pel) : -fabsf(*pel);
                    /*  *pel = sign(*pel,*pel1)  */
                }
        ...
        */
        ...
        snorm = Zero;
        for (l=0; l<m; l++) {
            snorm = MAX(snorm, MAX(fabsf(*(s+l)), fabsf(*(e+l))));
        }
        ...
        *	kase = 4     if er(m-1) is negligible (convergence).
            */
            ...
            mm2 = m - 2;
            for (l=mm2; l>=0; l--) {
                test = fabsf(*(s+l)) + fabsf(*(s+l+1));
                ztest = fabsf(*(e+l));
                if (!svd_IsFinite32(test) || !svd_IsFinite32(ztest)) {
                    info = -1;
                    return(info);
                }
            ...
            if (l == mm2) {
                ...
                for (ls=m; ls>lp1; ls--) {
                    test = Zero;
                    if (ls != m) test += fabsf(*(e+ls-1));
                    if (ls != l + 2) test += fabsf(*(e+ls-2));
                    ztest = fabsf(*(s+ls-1));
                    if (!svd_IsFinite32(test) || !svd_IsFinite32(ztest)) {
                        return(info);
                    }
            ...
            */
            ...
            case 3:				/* perform one qr step. */
                ...
                *   Calculate the shift.
                */
                scale = MAX(fabsf(*(s+mm1)), MAX(fabsf(*(s+mm2)), fabsf(*(e+mm2))));
                scale = MAX(fabsf(scale), MAX(fabsf(*(s+l)), fabsf(*(e+l))));
                sm = *(s+mm1) / scale;
                smm1 = *(s+mm2) / scale;
                emm1 = *(e+mm2) / scale;
                sl = *(s+l) / scale;
