...
static double infinity() {
    return bits_to_double(0x7ff0000000000000ull);
}
static double nan() {
    return bits_to_double(0x7ff8000000000000ull);
}
static float nanf() {
  return bits_to_float(0x7fc00000ul);
}
#endif
...
static int isfinite(double x){
    uint64_t exponent = (double_to_bits(x) >> 52) & 0x7ff;
    uint64_t mantissa = double_to_bits(x) & 0xffffffffffffful;
...
static double fabs(double n){
    return bits_to_double(double_to_bits(n) & 0x7fffffffffffffffull);
}
#else
...
static float fabsf(float n) {
  return bits_to_float(float_to_bits(n) & 0x7ffffffful);
}
...
static double _truncate(double x){
...
static double modf(double x, double *int_part){
    double ip;
    if(isnan(x)){
        *int_part = NAN;
        return NAN;
...
static double exp(double x){
    ...
    return bits_to_double((((int64_t)i + 1023) & 0x7ff) << 52) * 
...
static int is_negative(double x) {
  return double_to_bits(x) & 0x8000000000000000ull;
}
static int is_negativef(float x) {
  return float_to_bits(x) & 0x80000000ul;
}

#define SQRT(T, suffix, minus, plus)                          \
  T sqrt##suffix(T n){                                        \
      T square, x, old;                                       \
      int64_t val_int = T##_to_bits(n);                       \
      /* TODO replace with n < 0 */                           \
      if(is_negative##suffix(n)) {                            \
          errno = EDOM;                                       \
          return NAN##suffix;                                 \
      }                                                       \
      /*approximate square root by manipulating bits*/        \
      val_int -= ONE << minus; /* Subtract 2^m. */            \
      val_int >>= 1; /* Divide by 2. */                       \
      val_int += ONE << plus; /* Add ((b + 1) / 2) * 2^m. */  \
      /*refine approximation using newtons method*/           \
      x = bits_to_##T(val_int);                               \
      do {                                                    \
          old = x;                                            \
          x = (x + n/x) * 0.5##suffix;                        \
      /* TODO replace with != */                                           \
...
static double _cos_kernel(double x){
...
static double _tan_kernel(double x){
...
static double _tan_reduce(double x){
