...
from types import *
from chips.compiler import scaled_byte_addr, scaled_word_addr
...
class Trace:
    ...
    @classmethod
    def from_components(cls, lineno, filename):
        parser = lambda x: x
        parser.tokens = lambda x: x
        parser.tokens.lineno = lineno
        parser.tokens.filename = filename
        parser.function = None
        parser.global_scope = None
...
class Process:
    ...
    def generate(self):
        ...
        if global_size:
            instructions.append(
                {"trace": self.trace,
                 ...
                 "z": tos,
                 "a": tos,
                 "literal": scaled_byte_addr(global_size)})
        offset = 0
        ...
        return AsmProgram([], instructions, globals_size_bytes)
...
class Function:
    ...
    def generate(self):
        ...
        instructions.append({
            ...
            "z": tos,
            "a": tos,
            "literal": scaled_word_addr(self.offset),
        })
    ...
    def generate(self):
        ...
        if hasattr(self, "expression"):
            ...
            store_object(
                ...
                offset=scaled_word_addr(self.function.return_pointer),
                local=False
            )
        _return(self.trace, instructions)
            ...
            if is_array_of(self):
                for index, expression in enumerate(flatten(self.initializer)):
                    ...
                    store_object(
                        ...
                        offset=scaled_word_addr(self.offset) +
                        (index * scaled_byte_addr(size_of(expression))),
                        local=True
                    )
            else:
        ...
        if size_of(self) == 4:
            ...
            instructions.append(
                {"trace": self.trace,
                 ...
                 "z": tos,
                 "a": tos,
                 "literal": scaled_byte_addr(-size_of(self))})
        return instructions
    ...
    def generate(self):
        ...
        if reverse_operands:
            ...
            if is_pointer_to(self.left) and not is_pointer_to(self.right):
                ...
                if scaled_byte_addr(size) > 1:
                    instructions.append(
                        {"trace": self.trace,
                         "op": "literal",
                         "z": temp,
                         "literal": scaled_byte_addr(size)})
                    instructions.append(
                        {"trace": self.trace,
                         "op": "multiply",
                         "z": result,
        ...
        if is_pointer_to(self.right) and is_pointer_to(self.left):
            size = type_size(self.right.type_().base_type())
            if scaled_byte_addr(size) > 1:
                locations = scaled_byte_addr(size)
...
class FunctionCall(Expression):
    ...
    def generate(self):
        ...
        instructions.append({
            ...
            "z": tos,
            "a": tos,
            "literal": scaled_byte_addr(-argument_size),
        })
    ...
    def address(self):
        ...
        instructions.append(
            {"trace": self.trace,
             ...
             "z": result,
             "a": result,
             "literal": scaled_word_addr(self.struct_offset)})
        return instructions
...
class ArrayIndex(Object):
    ...
    def address(self):
        ...
        if scaled_byte_addr(size_of(self)) > 1:
            instructions.append(
                {"trace": self.trace,
                 "op": "literal",
                 "z": temp,
    ...
    def address(self):
        ...
        if self.instance.local:
            instructions.append({
                ...
                "z": result,
                "a": frame,
                "literal": scaled_word_addr(self.instance.offset)})
        else:
            instructions.append({
                "trace": self.trace,
                "op": "literal",
